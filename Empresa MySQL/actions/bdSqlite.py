...
class EmpresaBD():
    ...
    def connect(self):
        try:
            self.connection = sqlite3.connect('empresa.sqlite')
    ...
    def editarFuncBD(self,mat,nome,salario,matEdit):
        self.connect()
        try:
            querry = '''UPDATE Funcionarios SET matricula=?, nome=?,salario=? WHERE matricula=?;'''
    ...
    def excluirFuncBD(self,mat):
        self.connect()
        try:
            querry = '''DELETE FROM Funcionarios WHERE matricula=?;'''
    ...
    def pesquisarFuncBD(self,option,value):
        ...
        try:
            if option==1:
                querry = '''SELECT * FROM Funcionarios WHERE matricula=?;'''
                self.cursor.execute(querry,(value,))
            elif option==2:
                querry = '''SELECT * FROM Funcionarios WHERE nome=?;'''
                self.cursor.execute(querry,(value,))
            else:
...
bd = EmpresaBD()
bd.cadastrarFuncBD(2,'Gustavo',7000)
